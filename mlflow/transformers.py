...
def _get_default_pipeline_signature(
    ...
    if example:
        try:
            predictions = generate_signature_output(pipeline, example, inference_config)
...
def _infer_signature(**kwargs):
    """
    For the transformer flavor, the signature inference logic is part of the `save_model` API.
    """
    return None
